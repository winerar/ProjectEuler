...
{
    public class LargestProductInAGrid : IProblem
    {
        public int Id => 11;

        public string Title => "Largest product in a grid";

        public string Description => "In the 20×20 grid below, four numbers along a diagonal line have been marked in red." +
        ...
        public string Solution => $"the greatest product equals {FindGreatestProduct()}";

        private object FindGreatestProduct()
        {
            ...
            for (int i = 0; i < grid.Length; i++)
            {
                for (int j = 0; j < grid[i].Length; j++)
                {
                    ...
                    {
                        var horizontalProduct = grid[i][j] * grid[i][j + 1] * grid[i][j + 2] * grid[i][j + 3];
                        greatestProduct = greatestProduct < horizontalProduct ? horizontalProduct : greatestProduct;
                    }
                    if (i + 3 < grid.Length)
                    {
                        var verticalProduct = grid[i][j] * grid[i + 1][j] * grid[i + 2][j] * grid[i + 3][j];
                        greatestProduct = greatestProduct < verticalProduct ? verticalProduct : greatestProduct;
                    }
                    if (i + 3 < grid.Length && j + 3 < grid[i].Length)
                    {
                        var diagonal1Product = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] * grid[i + 3][j + 3];
                        var diagonal2Product = grid[i + 3][j] * grid[i + 2][j + 1] * grid[i + 1][j + 2] * grid[i][j + 3];
                        greatestProduct = greatestProduct < diagonal1Product ? diagonal1Product :
                            greatestProduct < diagonal2Product ? diagonal2Product : greatestProduct;
                    }
                }
            }
